extends Control
# Terms Navigation - Manages terms and conditions screen
...
func _ready() -> void:
	...
	_back_button = get_node_or_null("Panel/Panel4/backs2")
	if _back_button == null:
		_back_button = get_node_or_null("Panel/Panel4/Button2")
	if _back_button == null:
		_back_button = get_node_or_null("BackButton")
	
	# Check if coming from signup
	if get_tree().root.has_meta("terms_from_signup"):
		_is_from_signup = get_tree().root.get_meta("terms_from_signup")
