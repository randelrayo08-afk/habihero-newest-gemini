...
3. **Configure environment variables**:
   ...
   In PowerShell, use `Copy-Item .env.example .env`. The Daily Python package
   does not provide a Windows wheel, so local `uv sync` may fail on Windows.
   Pipecat Cloud builds the bot in a Linux environment, so this does not block
   cloud deployment.
